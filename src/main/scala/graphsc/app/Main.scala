...
//      graph.autoTransformations ::= graph.doNormalize
    ...
    if(conf.resid()) {
      ...
      performResidualization(graph, conf, resid, resid_tests, output)
    }
  ...
  def performResidualization(graph: MainHypergraphImplementation, conf: Conf,
                             ...
                             resid_tests: List[(RenamedNode, List[Value])],
    ...
    if(conf.residAutoTest.isSupplied) {
      if(conf.verbose())
        System.err.println("Running autogenerated tests...")
      for((n, vs) <- resid_tests)
